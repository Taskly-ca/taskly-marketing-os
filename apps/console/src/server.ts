/**
 * THE CONSOLE — the API behind TMOS's app.
 *
 * It began as TMOS with a face: `app.html` (the answer engine) and `ui.html`
 * (the dashboard) served from here. Since 2026-09-14 the face is apps/web —
 * Ask and Watch, behind a sign-in — and this process serves only the JSON and
 * streaming routes that app calls. `pnpm console` runs the two together.
 *
 * ── ON RAILWAY IT NEVER FACES THE NETWORK ──────────────────────────────────
 *
 * `scripts/start-railway.mjs` sets `TMOS_CONSOLE_BIND=127.0.0.1`, so inside the
 * container only the web app can reach it; the web app checks the session and
 * attaches the password below server-side.
 *
...
async function route(req: IncomingMessage, res: ServerResponse): Promise<void> {
  ...
  if (path === '/' || path === '/index.html') {
    /**
     * NO PAGES HERE ANY MORE (2026-09-14). The answer engine (`app.html`) and
     * the dashboard (`ui.html`, at `/classic`) were rebuilt as apps/web — Ask
     * and Watch — and deleted once every surface had a home there. This
     * process is the API the web app calls; `pnpm console` runs both.
     */
    return text(res, 200, 'TMOS console API. The app is apps/web — run `pnpm console` and open http://127.0.0.1:4480.');
  }
